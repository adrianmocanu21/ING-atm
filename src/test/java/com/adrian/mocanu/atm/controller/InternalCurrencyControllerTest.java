...
class InternalCurrencyControllerTest {
	...
	void controllerWorksFineOnValidRequest() throws Exception {
		...
		this.mockMvc
				.perform(post(PATH)
						.contentType(MediaType.APPLICATION_JSON).content(pairAsString))
				.andExpect(status().isCreated())
				.andExpect(
						jsonPath("$.[*].billDenomination").value(hasItems("1", "2", "3")))
	...
	void errorIsThrownOnDuplicateKey() throws Exception {
		...
		this.mockMvc
				...
				.andExpect(jsonPath("$.errorMessage").value(
						"JSON parse error: Denomination 30 is duplicated; nested exception is com.fasterxml.jackson.databind.JsonMappingException: Denomination 30 is duplicated (through reference chain: com.adrian.mocanu.atm.model.ValidatedHashMap[\"30\"])"));
	}

	@Test
	void errorIsThrownOnNonConvertibleKey() throws Exception {
		String notConvertibleKey = TestUtils
				.convertResourceToString(notConvertibleKeyJson);
		...
		this.mockMvc
				...
				.andExpect(jsonPath("$.title").value("Bad Request"))
				.andExpect(jsonPath("$.errorMessage").value(
						"JSON parse error: Denomination notConvertibleKey is not of type integer; nested exception is com.fasterxml.jackson.databind.JsonMappingException: Denomination notConvertibleKey is not of type integer (through reference chain: com.adrian.mocanu.atm.model.ValidatedHashMap[\"notConvertibleKey\"])"));
	}

	@Test
	void errorIsThrownOnExceededDenominationValue() throws Exception {
		String notConvertibleKey = TestUtils
				.convertResourceToString(exceededDenominationJson);
		...
		this.mockMvc
				...
				.andExpect(jsonPath("$.title").value("Bad Request"))
				.andExpect(jsonPath("$.errorMessage").value(
						"JSON parse error: Denomination 1001 must be smaller than 1000; nested exception is com.fasterxml.jackson.databind.JsonMappingException: Denomination 1001 must be smaller than 1000 (through reference chain: com.adrian.mocanu.atm.model.ValidatedHashMap[\"1001\"])"));
	}
	...
	void errorIsThrownOnEmptyJson() throws Exception {
		...
		this.mockMvc
				...
				.andExpect(jsonPath("$.errorMessage")
						.value("addCurrency.pairs: size must be between 1 and 100"));
	}
